...
def filter_flags(obj, f_in, f_ex): # recursively filter an object or list with match_flags
	if type(obj) == list: # bool filter discards empty elements
		return list(filter(bool, (filter_flags(entry, f_in, f_ex) for entry in obj)))
...
def tex_list(lis): # convert string list to LaTeX list
...
def format_list(lis, formatf): # converet list of objects to LaTeX string
	# lis: list<dict>; section to format
	# formatf: string; path to format file for individual element
...
def format_skills(obj, format_dir):
	return f'\\begin{{LIST}}\n{tex_list(skill["skill"] for skill in obj)}\n\\end{{LIST}}'
...
def gen_tex(order, infile, format_dir, flags_include, flags_exclude):
	...
	for c in order:
		sec = format_funcs[c](data[names[c]], format_dir)
		if sec:
